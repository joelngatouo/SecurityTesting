...
print("
 ...
 <form name='grades' action='./index.php' method='POST'>
  ...
  $query = mysql_query("SELECT assignmentid, title FROM assignments WHERE courseid = $trimmeselectclass ORDER BY duedate DESC")
  ...
  while($assignment = mysql_fetch_row($query))
  {
   ...
   $q = mysql_query("SELECT gradeid, points, submitdate, islate, comment FROM grades WHERE studentid = '$studentid' AND courseid = '$trimmeselectclass' AND assignmentid = '$assignment[0]'")
