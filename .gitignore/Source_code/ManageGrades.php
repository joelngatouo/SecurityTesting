<?php
 ...
 $trimmeselectclass=intval($_POST['selectclass']);
  ...
  else
  {
   ...
   $query = mysql_query("INSERT INTO grades VALUES('', '$_POST[assignment]', '$trimmeselectclass', '$ids[0]', '$ids[1]', '$_POST[student]', '$_POST[points]', '$_POST[comment]', '".converttodb($_POST['gradedate'])."', '".($_POST['late']==1 ? "1" : "0")."')")
  ...
  $query = mysql_query("SELECT semesterid FROM courses WHERE courseid = $trimmeselectclass");
  ...
  {
   if($wasdate < $middate)
	$query = mysql_query("UPDATE registrations SET q1currpoints=(q1currpoints + $_POST[points] - $_POST[wasgrade]), currentpoints=(currentpoints + $_POST[points] - $_POST[wasgrade]) WHERE courseid = $trimmeselectclass AND studentid = $_POST[student]");
   else
	$query = mysql_query("UPDATE registrations SET q2currpoints=(q2currpoints - $_POST[wasgrade]), q1currpoints=(q1currpoints + $_POST[points]), currentpoints=(currentpoints + $_POST[points] - $_POST[wasgrade]) WHERE courseid = $trimmeselectclass AND studentid = $_POST[student]");
  }
  else
  {
   if($wasdate < $middate)
	$query = mysql_query("UPDATE registrations SET q1currpoints=(q1currpoints - $_POST[wasgrade]), q2currpoints=(q2currpoints + $_POST[points]), currentpoints=(currentpoints + $_POST[points] - $_POST[wasgrade]) WHERE courseid = $trimmeselectclass AND studentid = $_POST[student]");
   else
	$query = mysql_query("UPDATE registrations SET q2currpoints=(q2currpoints + $_POST[points] - $_POST[wasgrade]), currentpoints=(currentpoints + $_POST[points] - $_POST[wasgrade]) WHERE courseid = $trimmeselectclass AND studentid = $_POST[student]");
  }
 }
 ...
 if($_POST["deletegrade"] == 1)
 {
  ...
  for($i=0; $i<sizeof($delete); $i++)
  {
   ...
   if($submitdate < $middate)
   {
	$query = mysql_query("UPDATE registrations SET q1currpoints=(q1currpoints - $id[1]), currentpoints=(currentpoints - $id[1]) WHERE studentid = $id[3] AND courseid = $trimmeselectclass");
   }
   else
   {
	$query = mysql_query("UPDATE registrations SET q2currpoints=(q2currpoints - $id[1]), currentpoints=(currentpoints - $id[1]) WHERE studentid = $id[3] AND courseid = $trimmeselectclass");
   }
  }
 }
...
print("<script language='JavaScript'>
 ...
 $query = mysql_query("SELECT * FROM assignments WHERE courseid = '$trimmeselectclass'");
 ...
 $query = mysql_query("SELECT assignmentid,title FROM assignments WHERE courseid = '$trimmeselectclass'")
...
print("  </select>
  ...
  $query = mysql_query("SELECT DISTINCT s.studentid, s.fname, s.lname FROM students s, registrations r WHERE s.studentid = r.studentid AND r.courseid = $trimmeselectclass ORDER BY UPPER(s.lname) ASC")
  ...
  while($student = mysql_fetch_row($query))
  {
   ...
   $q = mysql_query("SELECT gradeid, points, comment, submitdate, islate, studentid, comment FROM grades WHERE assignmentid = '$_POST[assignment]' AND courseid = '$trimmeselectclass' AND studentid = '$student[0]'")
   ...
   // Calculate and display the letter grade //
	$q = mysql_query("SELECT aperc,bperc,cperc,dperc,fperc FROM courses WHERE courseid = $trimmeselectclass") or die("ManageGrades.php: Unable to get the grade percentages - ".mysql_error());
